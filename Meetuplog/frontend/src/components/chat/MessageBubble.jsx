import { PencilIcon, ReplyIcon, TrashIcon } from "../common/Icons";
...
const getEmojiOnlyInfo = (content) => {
  ...
  if (GRAPHEME_SEGMENTER) {
    ...
    graphemes = Array.from(normalized).filter((part) => part.trim().length > 0);
  }

  if (
    graphemes.length === 0 ||
    graphemes.length > 5 ||
    !graphemes.every((part) => EMOJI_GRAPHEME_PATTERN.test(part))
  ) {
    return null;
  }
...
const truncate = (value, length = 52) => {
  if (!value) {
    return "";
  }
...
const MessageReactions = ({ message, currentUserId, onToggleReaction }) => {
  const reactions = message.reactions ?? {};

  const visible = Object.entries(reactions).filter(
...
const MessageBubble = ({
  ...
  if (message.type === "SYSTEM") {
    const eventType = message.systemEvent?.toLowerCase() ?? "info";
  ...
  const emojiOnlyInfo =
    !message.deleted && message.type === "TEXT" && !message.replyToId
      ? getEmojiOnlyInfo(message.content)
      : null;

  const emojiOnly = Boolean(emojiOnlyInfo);

  const emojiCountClass = emojiOnly
    ? `emoji-count-${Math.min(emojiOnlyInfo.count, 4)}`
    : "";
    ...
    >
      ...
      <div className="message-area">
        ...
        <div className="message-content-row">
          {mine && <MessageMeta message={message} />}

          <div className="message-bubble-shell">
            <div
              className={`message-bubble ${
                emojiOnly ? "emoji-only-bubble" : ""
              }`}
            ...
            >
              {!message.deleted &&
                ...
                >
                  ...
                  <span className="reply-preview-copy">
                    <strong>
                      {replyMessage ? replyMessage.senderName : "원문 메시지"}
                    </strong>
              ...
              <span
                className={[
                  "message-body-copy",
                  message.deleted ? "deleted-message-copy" : "",
                  emojiOnly ? "message-emoji-only-content" : "",
                ].join(" ")}
              >
                {message.deleted ? (
                  "삭제된 메시지입니다."
                  ...
                  <span className="message-image-attachment">
                    <img
                      ...
                      alt={message.content || "전송한 이미지"}
                      loading="lazy"
                      decoding="async"
                    />

                    <span>{message.content || "사진"}</span>
                  </span>
                ) : (
                  message.content
                )}
            ...
            {selected && canInteract && (
              <div
                className={["message-action-menu", mine ? "mine" : ""].join(
                  " ",
                )}
              ...
              >
                <div className="message-quick-reactions">
                  {QUICK_REACTIONS.map((emoji) => {
                    const reacted = (reactions[emoji] ?? []).includes(
                      currentUserId,
                    );
