...
def _has_korean_display_title(movie: Movie) -> bool:
    if movie.title_ko and movie.title_ko.strip():
        return True

    return bool(re.search(r"[가-힣]", movie.title or ""))


def _movie_title_identity_keys(movie: Movie) -> set[str]:
    """Return stable-enough title keys for duplicate catalog rows.

    A single real movie can occasionally have separate TMDB/KOBIS rows with
    different internal IDs.  We therefore keep every localized/original title
    alias as an identity key in addition to the TMDB ID check performed by the
    caller.
    """
    ...
    return {
        ...
        if value and (normalized := _normalize_movie_title(value))
    }


def _normalize_certification(value: str | None) -> str:
    return re.sub(r"\s+", "", value or "").casefold()


def _is_adult_movie(movie: Movie) -> bool:
    certification = _normalize_certification(movie.certification)
    if certification in {
        _normalize_certification(value)
        for value in ADULT_CERTIFICATIONS
    }:
        return True

    metadata_terms = {
        str(value).strip().casefold()
        for value in [*movie.keywords, *movie.content_flags]
        if value
    }
    if any(
        term in metadata_terms
        for term in ADULT_CATALOG_TERMS
    ):
        return True
    ...
    return bool(
        re.search(
            r"(?:성적\s*판타지|성행각|성애\s*영화|"
            r"성인\s*영화|에로\s*영화|노출\s*연기|섹스)",
            text,
            re.IGNORECASE,
        )
    )
...
def has_verified_requested_certification(
    movie: Movie,
    members,
) -> bool:
    """Allow a KOBIS-rated row when rating safety is explicit and verified.

    Some KOBIS-only rows have no poster/overview and are marked ineligible by
    the normal completeness threshold.  When a user explicitly requests an
    age rating, a row with that exact official rating is safer than filling the
    card with an unrated title.
    """

    movie_certification = _normalize_certification(
        movie.certification
    )
    constrained_members = [
        member
    ...
    if (
        not movie_certification
        or not constrained_members
        or not (movie.kobis_code or movie.providers or movie.cinema_sources)
    ):
        return False
    ...
    return all(
        ...
        in {
            _normalize_certification(value)
            for value in member.certifications
        }
...
def hard_violations(
    ...
    if pref.certifications:
        allowed_certifications = {
            _normalize_certification(value)
            for value in pref.certifications
        }
        movie_certification = _normalize_certification(
            movie.certification
        )
        if not movie_certification:
            reasons.append(
                f"{pref.user_id}: 관람등급 정보 없음"
            )
        elif movie_certification not in allowed_certifications:
            reasons.append(
    ...
    if pref.countries:
        ...
        primary_country = (
            _country_code(movie.countries[0])
            if movie.countries
            else None
        )
        korean_origin_match = (
            "KR" in allowed_countries
            and (
                primary_country == "KR"
                or (not primary_country and movie.language == "ko")
            )
        )
        requires_strict_korean_origin = allowed_countries == {"KR"}

        if not movie_countries and not korean_origin_match:
            reasons.append(
                f"{pref.user_id}: 제작 국가 정보 없음"
            )

        elif (
            requires_strict_korean_origin
            and not korean_origin_match
        ) or (
            not requires_strict_korean_origin
            and not allowed_countries.intersection(movie_countries)
        ):
            reasons.append(
    ...
    for phrase in pref.hard_exclusions:
        exclusion_terms = {
            ...
            *(
                term.casefold()
                for term in HARD_EXCLUSION_ALIASES.get(
                    phrase,
                    set(),
                )
            ),
...
def recommend(
    ...
    previously_recommended = [
        movie
        for movie in movies
        if movie.internal_id in reroll_exclusions
    ]
    excluded_tmdb_ids = {
        movie.tmdb_id
        for movie in previously_recommended
        if movie.tmdb_id is not None
    }
    excluded_title_keys: set[str] = set()
    for movie in previously_recommended:
        excluded_title_keys.update(
            _movie_title_identity_keys(movie)
        )
    ...
    for movie_index, movie in enumerate(
        ...
        if not _has_korean_display_title(movie):
            excluded.append(
                {
                    "movie_id": movie.internal_id,
                    "title": movie.title,
                    "reasons": ["한국어 표시 제목 미등록"],
                }
            )
            continue

        if (
            movie.internal_id in reroll_exclusions
            or (
                movie.tmdb_id is not None
                and movie.tmdb_id in excluded_tmdb_ids
            )
            or bool(
                _movie_title_identity_keys(movie)
                & excluded_title_keys
        ...
        if not request.require_now_playing:
            available_in_korea = bool(
                movie.providers
                or movie.is_now_playing
                or movie.kobis_code
                or movie.cinema_sources
            )

            if (
                not request.include_unknown_watch_path
                and not available_in_korea
            ):
                excluded.append(
                    {
                        "movie_id": movie.internal_id,
                        "title": movie.title,
                        "reasons": ["한국 내 OTT 제공·상영·개봉 정보 없음"],
                    }
                )
                continue
    ...
    if (
        ...
        fallback_stages = [
            ...
            (
                ...
                "현재 상영 조건을 완화해 한국 개봉·OTT 작품 3편 구성",
            ),
        ]
