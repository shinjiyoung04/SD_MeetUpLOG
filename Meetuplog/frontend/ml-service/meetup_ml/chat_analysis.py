...
def _apply_effect(
    ...
    if kind == "GENRE":
        ...
        state["certifications"] = [str(target)] if value > 0 else []
...
def analyze_chat(
    ...
    for message in request.messages:
        ...
        explicit_person_name_context = bool(
            detected_people
            and any(
                len(_clean(name)) >= 4
                and _clean(name) in _clean(message.text)
                for name, _role in detected_people
            )
        )

        if person_request_context or explicit_person_name_context:
            movie_score = 0.0
            movie_title = None
            original_token = None
            movie_id = None
        ...
        all_ages_intent = bool(
            re.search(
                r"(?:전체\s*관람(?:가|가로|자로?)|"
                r"전\s*연령|모든\s*연령|누구나\s*볼\s*수)",
                message.text,
            )
        )
        if all_ages_intent:
            # 관람등급이 명시되면 이후 추천을 다시 눌러도 누적 상태에
            # 남아 있도록 사용자 선호 프로필에 즉시 저장한다.
            state["certifications"] = ["전체관람가"]
        ...
        if all_ages_intent:
            ...
            if preference_score > 0:
                ...
                if (
                    preference_score <= -0.6
                    and topic not in state["hard_exclusions"]
                ):
                    state["hard_exclusions"].append(topic)
        ...
        if genre and target_type != "GENRE":
            ...
            genre_like = bool(
                re.search(
                    ...
                    rf"(?:좋아|좋음|좋고|선호|보고\s*싶|재밌|잘\s*봐|잘봐|잘\s*보|잘봄)",
                    message.text,
                    re.IGNORECASE,
                )
                or re.search(
                    rf"(?:좋아|선호|재밌|보고\s*싶|잘\s*봐|잘봐|잘\s*보|잘봄).{{0,12}}"
                    rf"(?:{genre_expression})",
                    message.text,
                    re.IGNORECASE,
                )
