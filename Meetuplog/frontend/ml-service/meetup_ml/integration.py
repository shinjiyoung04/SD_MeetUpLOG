...
def integrate_kobis(store: JsonStore, threshold: float = .78) -> dict:
    ...
    for movie in movies:
        ...
        kobis_title = (info.get("movieNm") or "").strip()
        if kobis_title and any("가" <= character <= "힣" for character in kobis_title):
            movie.title_ko = kobis_title
            movie.title = kobis_title
