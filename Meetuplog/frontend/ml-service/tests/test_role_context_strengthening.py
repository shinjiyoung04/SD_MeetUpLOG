...
def test_role_preferences_change_scores_for_legacy_catalog_name_fields():
    """The current 5,000-title catalog has cast/director names but no person IDs yet."""
    preferred = _movie("preferred")
    preferred.cast = ["송강호"]
    avoided = _movie("avoided")
    avoided.directors = ["봉준호"]
    neutral = _movie("neutral")
    preference = Preference(
        user_id="A",
        liked_actors=[PersonPreference(name="송강호", role="ACTOR")],
        disliked_directors=[PersonPreference(name="봉준호", role="DIRECTOR")],
    )

    preferred_score = member_fit(preferred, preference)
    avoided_score = member_fit(avoided, preference)
    neutral_score = member_fit(neutral, preference)

    assert preferred_score.score > neutral_score.score
    assert avoided_score.score < neutral_score.score
    assert preferred_score.score_breakdown["liked_actors"] > 0
    assert avoided_score.score_breakdown["disliked_directors"] < 0
