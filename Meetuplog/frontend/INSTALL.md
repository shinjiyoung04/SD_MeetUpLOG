# MeetupLog Frontend v44 — 재가입·ML 추천 확정 연동

## v44 탈퇴 후 재가입 및 확정 영화 후속 동작

- 탈퇴 이메일의 SHA-256 지문과 비활성 레코드는 유지하면서 동일 이메일 재가입 허용
- 재가입 전 로그인은 `WITHDRAWN_ACCOUNT`, 재가입 후에는 새 활성 계정으로 정상 로그인
- 팀원 ML 서비스의 누적 채팅 분석·역할 기반 선호·추천 결과 보강 코드 반영
- 방장 확정 결과를 채팅 카드로 저장하고 모든 참여자에게 실시간 전파
- 확정 영화의 국내 OTT·영화관 경로와 바로 보기·예매 링크 표시
- 카카오 JavaScript SDK 기반 확정 결과 공유와 네이티브 공유·클립보드 대체 동작
- 확정 상태를 모든 참여자가 읽으면 기존 방 수명주기에 따라 24시간 뒤 방 삭제

카카오 공유는 `.env`에 `VITE_KAKAO_JAVASCRIPT_KEY`와 운영 주소인
`VITE_PUBLIC_APP_URL`을 설정해야 합니다.
